...
import {
  cloneOrFetchRepo, cloneRepo, checkoutSha, getWorkdirForRepoUrl,
  getTempdirForRepoUrl, getOriginForRepo, getHeadForRepo, resetOriginUrl
} from './git-api';
import {
  getSanitizedRepoUrl, getNwoFromRepoUrl, postCommitStatus, createGist,
  findPRForCommit
} from './github-api';
...
export function main(argv, showHelp): Promise<number> {
  ...
  return doIt
    .then((x) => Promise.resolve(x as number), (e) => {
...
async function realMain(argv, showHelp) {
  ...
  let r = await retryPromise(() => cloneRepo(bareRepoDir, workDir, null, false)) as any;
  r.free();
  ...
  try {
    ...
    buildStream.concatMap((x: string) => {
